...
while getopts 'L:l:m:t:s:F:i:y:nND' opt; do
    case $opt in
        L)
...
show_notify(){
    ...
    if [[ ${LIMIT} -eq ${LOWER_LIMIT} ]] ; then
      NOTI=$NOTIFY_ICON2
    else
      NOTI=$NOTIFY_ICON
    fi
    echo $NOTI
    [[ -n $NOTI ]] && NOTIFY_OPT="-i ${NOTI}"
    ~/programs/dunstify -a "Notifier" ${NOTIFY_OPT} "${1}"
}
